...
const getAllJobs = async (req, res) => {
  const { status, type, workArrangement, sort, search } = req.query;
  ...
  if (workArrangement && workArrangement !== 'all')
    queryObject.workArrangement = workArrangement;

  if (search) queryObject.position = { $regex: search, $options: 'i' };

  // no await because we still want to sort the query result
...
const showStats = async (req, res) => {
  ...
  const defaultStats = {
    'plan to apply': stats['plan-to-apply'] || 0,
